...
document.addEventListener("DOMContentLoaded", function() {
	...
	if(formProductos) {
		formProductos.addEventListener("submit",function(e) {
			...
			let precioCrudo = parseFloat(document.getElementById("precioProducto").value);
			let ivaCrudo = parseFloat(document.getElementById("ivaProducto").value);
			let precio = precioCrudo + (precioCrudo * ivaCrudo / 100);
			
			let registroProducto = {
				nombreProducto: document.getElementById("nombreProducto").value,
				stockProducto: document.getElementById("cantidadProducto").value,
				precioProducto: document.getElementById("precioProducto").value,
				ivaProducto: document.getElementById("ivaProducto").value,
				imagenProducto: document.getElementById("imgProducto").value,
				precioFinal: precio.toFixed(2),
				tipoProducto: document.getElementById("filtroTipo").value
			}
		...
		let producto = productos[i];
			catalogo += `
						<div class="card"> 
							...
							<button type="button" class="btn btn-primary" onclick="window.location.href='editar_producto_admin.html?
								nombre=${producto.nombreProducto}&
								stock=${producto.stockProducto}&
								precio=${producto.precioProducto}&
								img=${producto.imagenProducto}&
								iva=${producto.ivaProducto}&
								tipo=${producto.tipoProducto}'">Editar Producto</button> 
							<button type="button" class="btn btn-secondary" onclick="borrarProducto('${producto.nombreProducto}')">Borrar Producto</button>
						</div>`;
	}
