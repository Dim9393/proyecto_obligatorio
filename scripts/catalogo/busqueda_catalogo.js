...
document.addEventListener('DOMContentLoaded', function () {
	...
	let busquedaInexistente =
		'<center><h2>No hemos encontrado ningún producto...</h2></center>'

	let productosBuscados = 0

	for (let i = 0; i < productos.length; i++) {
		let producto = productos[i]
		//El .replaceAll(" ", "") saca los espacios para que no tengamos problemas a la hora de que nos busquen algo
		...
		if (nombre.includes(busquedaUser)) {
			busqueda += `<div class="card"> 
						...
						<button class="btn btn-primary" onclick="window.location.href='producto.html?
							...
							iva=${producto.ivaProducto}'">Ver detalles
						</button> 
						<br><br><br> 
						</div> `
			productosBuscados++
