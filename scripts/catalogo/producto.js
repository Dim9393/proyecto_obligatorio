...
document.addEventListener('DOMContentLoaded', function () {
	...
	if (formProducto) {
		formProducto.addEventListener('submit', function (e) {
			...
			let cantidadAlCarrito = Number(
				document.getElementById('agregarCantProducto').value,
			)
			...
			if (cantidadAlCarrito <= stockNombre && cantidadAlCarrito > 0) {
				...
				for (let i = 0; i < productos.length; i++) {
					...
					//Guardo en Storage la cantidad que la persona quiso, en este caso quitandole al stock del producto
				...
				for (let i = 0; i < productosCarrito.length; i++) {
					...
					if (productoCarrito.nombre === agregarProducto.nombre) {
						...
						productoCarrito.precio =
							Number(agregarProducto.precio) + Number(productoCarrito.precio)
						repetido = true
						break
					}
				}
	...
	for (let i = 0; i < productos.length; i++) {
		...
		if (producto.nombreProducto === productoNombre) {
			if (producto.stockProducto == 0) {
				document.getElementById('faltaStock').innerHTML = `
				<button class="btn btn-secondary" type="button" onclick="alert('¡Se ha informado!')">¡Informar falta de stock!</button>`
				return
			}
		}
	}
