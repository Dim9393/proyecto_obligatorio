...
function mostrarCarrito() {
	...
	for(let i = 0; i < productosCarrito.length; i++) {
		...
		carrito += `<div class="card"> 
						...
						<button type="button" class="btn btn-primary" onclick="window.location.href='editar_producto_user.html?
						nombre=${productoCarrito.nombre}&
						cantidadUser=${productoCarrito.cantidad}&
						precio=${productoCarrito.precio}&
						img=${productoCarrito.foto}'"
						>Editar Producto</button> 
						<button type="button" class="btn btn-secondary" onclick="borrarProducto('${productoCarrito.nombre}')">Eliminar Producto</button>
						<br><br><br> 
					</div>`
	}
	let total = `<div class="card"> <hr> 
...
function borrarCarrito() {
	...
	if (verfCarrito) {
		//Se devuelve el stock que la persona elimino de su carrito
		for(let i = 0; i < productosCarrito.length; i++) {
			let productoCarrito = productosCarrito[i];
		...
		//Recarga la pagina para que el carrito desaparezca, lo vimos con Pablo
...
//Lo que buscamos con esta funcion es borrar el producto del carrito del usuario y devolverlo al stock de la tienda
...
function continuarCompra() {
	...
	if(productosCarrito.length > 0) {
		continuar = `<button class="btn btn-primary" onclick="window.location.href='confirmar_compra.html'">Continuar</button>`
	}
